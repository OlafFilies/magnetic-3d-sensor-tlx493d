...
TEST(TLx493D_A1B6, setter_BitFields)
{
    ...
    // for gen1 setBitField is used to set values of WRITE registers (BitFields) only (due to a check for WRITE_MODE_e access)
    // so we test this function only for WRITE BitFields 
    for(uint8_t i=0; i < GEN_1_BITFIELDS_COUNT; i++){
        Register_ts *bf = &dut.regDef[i];
...
TEST(TLx493D_A1B6, getter_BitFields)
{
    ...
    // for gen1 returnBitField is used to get values of READ registers (BitFields) only (due to a check for READ_MODE_e access)
