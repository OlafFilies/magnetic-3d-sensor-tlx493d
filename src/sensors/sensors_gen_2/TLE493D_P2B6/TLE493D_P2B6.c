...
/*
  ...
  Used to index "TLE493D_P2B6_regDef" defined below, so index values must match !
*/
typedef enum {
               BX_MSB = 0,
               BY_MSB,
               ...
               PRD,
               TYPE,
               HWV } TLE493D_P2B6_registerNames_te;


Register_ts TLE493D_P2B6_regDef[] = {
...
typedef enum {
               ...
               RESERVED_MODE          = 0x10,
               FAST_MODE              = 0x11 } TLE493D_P2B6_modes_te;
...
CommonFunctions_ts TLE493D_P2B6_commonFunctions = {
                                .init                 = TLE493D_P2B6_init,
                                .deinit               = TLE493D_P2B6_deinit,

                                .getTemperature       = TLE493D_P2B6_getTemperature,

                                .calculateFieldValues = TLE493D_P2B6_calculateFieldValues,
                                .getFieldValues       = TLE493D_P2B6_getFieldValues,

                                .setDefaultConfig     = TLE493D_P2B6_setDefaultConfig,
...
*/
bool TLE493D_P2B6_init(Sensor_ts *sensor) {
    ...
    sensor->regDef            = TLE493D_P2B6_regDef;
    ...
    sensor->functions         = &TLE493D_P2B6_commonFunctions;
    sensor->regMapSize        = GEN_2_REG_MAP_SIZE;
    sensor->sensorType        = TLE493D_P2B6_e;
 ...
 * 
*/
bool TLE493D_P2B6_deinit(Sensor_ts *sensor) {
    free(sensor->regMap);
 ...
 * 
*/
void TLE493D_P2B6_calculateTemperature(Sensor_ts *sensor, float *temp) {
    int16_t value = 0;
 ...
 * 
*/
bool TLE493D_P2B6_getTemperature(Sensor_ts *sensor, float *temp) {
    if( gen_2_readRegisters(sensor) ) {
        TLE493D_P2B6_calculateTemperature(sensor, temp);
        return true;
    }
 ...
 * 
*/
void TLE493D_P2B6_calculateFieldValues(Sensor_ts *sensor, float *x, float *y, float *z) {
 ...
 * 
*/
bool TLE493D_P2B6_getFieldValues(Sensor_ts *sensor, float *x, float *y, float *z) {
    if( gen_2_readRegisters(sensor) ) {
        TLE493D_P2B6_calculateFieldValues(sensor, x, y, z);
        return true;
    }
 ...
 * 
*/
bool TLE493D_P2B6_setDisableTemperatureMeasurements(Sensor_ts *sensor, uint8_t dt) {
    uint8_t config = sensor->commonRegisters.CONFIG;

    // CONFIG register
    gen_2_setBitfield(sensor, DT, dt);
    gen_2_setBitfield(sensor, CP, TLE493D_P2B6_calculateConfigurationParity(sensor));

    gen_2_writeRegister(sensor, DT);
}


bool TLE493D_P2B6_enableTemperatureMeasurements(Sensor_ts *sensor) {
    return TLE493D_P2B6_setDisableTemperatureMeasurements(sensor, 0);
}


bool TLE493D_P2B6_disableTemperatureMeasurements(Sensor_ts *sensor) {
    return TLE493D_P2B6_setDisableTemperatureMeasurements(sensor, 1);
}
 ...
 * 
*/
bool TLE493D_P2B6_set1ByteReadMode(Sensor_ts *sensor, uint8_t pr) {
...
bool TLE493D_P2B6_enable1ByteMode(Sensor_ts *sensor) {
     return TLE493D_P2B6_set1ByteReadMode(sensor, 1);
}


bool TLE493D_P2B6_disable1ByteMode(Sensor_ts *sensor) {
     return TLE493D_P2B6_set1ByteReadMode(sensor, 0);
}
...
*/
bool TLE493D_P2B6_setDefaultConfig(Sensor_ts *sensor) {
    ...
    if( TLE493D_P2B6_enable1ByteMode(sensor) ) {
        if( TLE493D_P2B6_enableTemperatureMeasurements(sensor) ) {
            ...
            // if( gen_2_readRegisters(sensor) )
            //     return TLE493D_P2B6_setLowUpdateRate(sensor);
