...
typedef enum {
               ...
               CHIP_ID_4,
               ID_PAR,
               CHIP_ID_5 } TLE493D_P3I8_registerNames_te;


Register_ts TLE493D_P3I8_regDef[] = {
...
typedef enum {
               ...
               RESERVED_MODE          = 0x10,
               FAST_MODE              = 0x11 } TLE493D_P3I8_modes_te;
...
CommonFunctions_ts TLE493D_P3I8_commonFunctions = {
                                .init                 = TLE493D_P3I8_init,
                                .deinit               = TLE493D_P3I8_deinit,

                                .getTemperature       = TLE493D_P3I8_getTemperature,

                                .calculateFieldValues = TLE493D_P3I8_calculateFieldValues,
                                .getFieldValues       = TLE493D_P3I8_getFieldValues,

                                .setDefaultConfig     = TLE493D_P3I8_setDefaultConfig,
                                .readRegisters        = gen_3_readRegistersSPI,
};
...
*/
bool TLE493D_P3I8_init(Sensor_ts *sensor) {
    ...
    sensor->regDef            = TLE493D_P3I8_regDef;
    sensor->functions         = &TLE493D_P3I8_commonFunctions;
    sensor->regMapSize        = GEN_3_REG_MAP_SIZE;
    sensor->sensorType        = TLE493D_P3I8_e;
    sensor->comIFType         = SPI_e;
 ...
 * 
*/
bool TLE493D_P3I8_deinit(Sensor_ts *sensor) {
    free(sensor->regMap);
 ...
 * 
*/
void TLE493D_P3I8_calculateRawTemperature(Sensor_ts *sensor, float *temp) {
    int16_t value = 0;
 ...
 * 
*/
void TLE493D_P3I8_calculateTemperature(Sensor_ts *sensor, float *temp) {
    int16_t value = 0;
 ...
 * 
*/
bool TLE493D_P3I8_getTemperature(Sensor_ts *sensor, float *temp) {
    if( gen_3_readRegistersSPI(sensor) ) {
        TLE493D_P3I8_calculateTemperature(sensor, temp);
        return true;
    }
 ...
 * 
*/
void TLE493D_P3I8_calculateFieldValues(Sensor_ts *sensor, float *x, float *y, float *z) {
...
// TODO: TLE493D_P3I8_calculateTemperatureRaw hinzufügen um die LSB Werte für Temp zu bekommen !
double r    = 1.0;
// double temp = 0.0;
float temp = 0.0;
double sensitivity = GEN_3_FULL_RANGE_FIELD_SENSITIVITY;
    
    TLE493D_P3I8_calculateRawTemperature(sensor, &temp);
 ...
 * 
*/
bool TLE493D_P3I8_getFieldValues(Sensor_ts *sensor, float *x, float *y, float *z) {
    if( gen_3_readRegistersSPI(sensor) ) {
        TLE493D_P3I8_calculateFieldValues(sensor, x, y, z);
        return true;
    }
...
*/
bool TLE493D_P3I8_setDefaultConfig(Sensor_ts *sensor) {
    ...
    gen_3_setBitfield(sensor, INT_DIS, 1);
    gen_3_setBitfield(sensor, WU_EN, 0);
    gen_3_setBitfield(sensor, CRC_WR_EN, 0);

    gen_3_writeRegisterSPI(sensor, MODE_SEL);
